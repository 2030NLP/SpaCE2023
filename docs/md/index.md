...
- results：异常文本片段。为了描述空间语义异常，最多可以选取 6 个文本片段，最少可以选取 1 个。每个文本片段包含 3 个字段，分别是角色（role）、文本内容（text）和字序数组 （idxes）。role的取值包括`S1 P1 E1 S2 P2 E2`，当文本片段个数不大于3时，role的取值包括`S1 P1 E1`。异常文本片段的选取有以下六种情况：
  ...
  - ③ 4 个文本片段。这些片段形成两个不完整的S-P-E三元组，各缺省一个role，两个role在文本中没有出现。如`他钻了进去，于是我也钻了出来，与他大打一场。`，该句没有出现表达P信息，所以不标注P1和P2。异常文本片段是`S1 他, E1 钻进去, S2 我, E2 钻出来`。
  ...
  - ⑤ 2 个文本片段。这些片段形成一个不完整的S-P-E三元组，缺省了某个role。具体有以下两种情况：
    ...
    - 不需要该role便可以描述异常。如`他可以从缝隙钻进屋子外`，该异常发生在`钻进屋子外`，与空间实体`他`无关，所以不标注S1。异常文本片段是`P1 屋子外, E1 钻进`。
...
子任务 2 的**输出**包含 2 个部分：
- qid：数据编号；
- results：空间实体及其空间方位信息，描述了“某空间实体在某时，经由某事件，满足某种空间方位关系，这一命题的事实性为真/假”的信息。共有 **15** 个语义角色（role）可供标注，如表1所示。文本片段型角色需要记录文本内容（text）和字序数组（idxes），标签型角色需要记录标签的值（label）。 没有出现的角色不需要标注。
...
该任务要求参赛系统对输入的 context 使用上表的 15 个角色标注出现的所有空间信息。最终输出一个results元组，其中的每一项是一条空间信息，每条空间信息标注了所使用的元素。样例如下：
...
> { "qid":"2-train-21",
>   "results":[ 
>     [ {"role":"空间实体","text":"老妇人","idxes":[0,1,2]},
>       {"role":"事件","text":"跪","idxes":[3]},
>       {"role":"处所","text":"在那坑里","idxes":[4,5,6,7]} ],
>     [ {"role":"空间实体","text":"那些水","idxes":[13,14,15]},
>       {"role":"事件","text":"舀","idxes":[11]},
>       {"role":"处所","text":"在那坑里","idxes":[4,5,6,7]},
>       {"role":"方向","text":"出","idxes":[12]} ],
>     [ {"role":"空间实体","text":"水","idxes":[38]},
>       {"role":"事件","text":"舀","idxes":[40]},
>       {"role":"事实性","label":"假"},
>       {"role":"处所","text":"坑里","idxes":[35,36]} ],
>     [ {"role":"空间实体","text":"她","idxes":[46]},
>       {"role":"时间","text":"舀","idxes":[40],"label":"之后"},
>       {"role":"终点","text":"到那死者旁边","idxes":[48,49,50,51,52,53]},
>       {"role":"方向","text":"回","idxes":[47]} ],
>     [ {"role":"空间实体","text":"她","idxes":[46]},
>       {"role":"事件","text":"抄","idxes":[57]},
>       {"role":"处所","text":"在死者的腋窝下","idxes":[58,59,60,61,62,63,64]},
>       {"role":"部位","text":"两手","idxes":[55,56]} ] ] }
> ```
>
> 同指信息：
> 
...
- 模板一：两段文本表示相同的空间场景
> “两段文本的形式差异在于`<取自 context1 的字符串 = C1>`和`<取自 context2 的字符串 = C2>`。两个表述中都出现了以下空间实体：`<空间实体 = S>`。尽管两段文本在描述`<S>`的`<空间信息 {P1, P2, ...} (P1 ∈ C1, P2 ∈ C2)>`有形式差异，但实际上，`<P1>`和`<P2>`描述`<S>`的`<处所|终点|方向|朝向|形状|路径|距离>`是相同的。因此，在这种情况下，这两段文本可以描述相同的空间场景。” 

样例如下（数据集发布后将补充样例的qid字段）：
> 输入：
> 
> ```json
...
> {"results":[
>    {"judge":"true",
>     "reason":"两段文本的差异在于“围坐在圆桌前”和“围坐在圆桌边”。两个表述中都出现了以下空间实体：圆桌、李光头、忠臣、瘸子、傻子、聋子和瞎子。尽管两段文本在描述李光头等人相对于圆桌的位置上有形式差异，但实际上，“在圆桌前”和“在圆桌边”描述的处所是相同的。因此，在这种情况下，这两段文本可以描述相同的空间场景。"} ] }
> ```

- 模板二：两段文本表示不同的空间场景
> “两段文本的形式差异在于`<取自 context1 的字符串 = C1>`和`<取自 context2 的字符串 = C2>`。两个表述中都出现了以下空间实体：`<空间实体 = S>`。两段文本在描述`<S>`的`<空间信息 {P1, P2, ...} (P1 ∈ C1, P2 ∈ C2)>`存在的形式差异表明：`<P1>`和`<P2>`描述`<S>`的`<处所|终点|方向|朝向|形状|路径|距离>`是不同的。因此，在这种情况下，这两段文本不能描述相同的空间场景。” 
...
> {"results":[
>    {"judge": "false",
>     "reason":"两段文本的差异在于“转过去”和“转回来”。两个表述中出现了以下空间实体：爸爸、妈妈、奶奶、我。两段文本在描述我的方向上存在的形式差异表明：“过去”和“回来”描述“我”的方向是不同的。因此，在这种情况下，这两段文本不能描述相同的空间场景。"} ] }
> ```


#### 2.2  <span id="distribution">数据规模与分布<span>

子任务 1 约 7,000 条语料，子任务 2 约 2,000 条语料，均按 7:1:2 的比例划分了训练集、验证集和测试集。子任务 3 不划分数据集，只提供1/10的无答案生语料帮助参赛队伍理解子任务三的目标。
...
- ② 对待检元组和参考元组进行匹配。对于每个元组对，按照以下程序计算待检元组的得分：
  - 当 role 是“空间实体”或“参照实体”时，计算 idxes 字段的 *`F1`* 值；对于其他文本片段类的角色，计算 text 字段的 *`F1`* 值；对于标签类的角色，相同计 1 分，不同计 0 分。其中，空间实体类角色（1-2 号元素）的匹配将会参考 corefs 数组，求其可能获得的最高得分。当参考角色和待检角色都不出现，此角色不计分。当参考角色和待检角色有一个不出现，此角色也不计分。
...
更详细的说明请看：[task2-eval.md](https://github.com/2030NLP/SpaCE2023/blob/main/eval/task2-eval.md)

#### 3.3  子任务3的评价标准

子任务 3 首先根据 judge 字段的结果评价异同判断是否正确，如果错误，该题得 0 分；如果正确，采用人工评价对 reason 字段的生成文本进行打分。评分内容包括文本流畅度、解释准确性、是否准确识别空间实体、对空间方位关系的运用等方面。
